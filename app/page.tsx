...
const appUrl = "https://mirumi-96354.web.app";
const isAndroid = () => typeof navigator !== "undefined" && /Android/i.test(navigator.userAgent);
const isInAppBrowser = () => {
  if (typeof navigator === "undefined") return false;

  return /KAKAOTALK|Instagram|FBAN|FBAV|NAVER|Line|Twitter|DaumApps/i.test(navigator.userAgent);
};
...
export default function Home() {
  ...
  const [blockedBrowser, setBlockedBrowser] = useState(false);

  useEffect(() => {
    setBlockedBrowser(isInAppBrowser());
  ...
  async function login() {
    setAuthError("");
    if (blockedBrowser) {
      setAuthError("Google 로그인이 차단될 수 있는 앱 안 브라우저입니다. Safari나 Chrome에서 열어주세요.");
      return;
    }
  ...
  async function copyAppUrl() {
    ...
    setAuthError("주소를 복사했어요. Safari나 Chrome 주소창에 붙여넣어 열어주세요.");
  }

  function openExternalBrowser() {
    if (isAndroid()) {
      window.location.href = "intent://mirumi-96354.web.app/#Intent;scheme=https;package=com.android.chrome;end";
      return;
    }
    ...
    return (
      <main className="flex min-h-screen items-center justify-center bg-[#fff8df] p-6">
        <section className="w-full max-w-sm rounded-[28px] bg-[#fffdf4] p-8 text-center shadow-sm">
          ...
          {blockedBrowser && (
            <div className="mt-5 rounded-2xl bg-white p-4 text-left text-sm text-[#6b4a24] ring-1 ring-[#ead7ad]">
              <p className="font-bold">휴대폰 로그인 안내</p>
              <p className="mt-2 text-[#8a7358]">
                카톡, 인스타, 네이버앱 안에서 열면 Google 로그인이 403으로 차단될 수 있어요. Safari나 Chrome에서 다시 열어주세요.
              </p>
              <div className="mt-4 grid grid-cols-2 gap-2">
                <button onClick={openExternalBrowser} className="rounded-xl bg-[#ffd75d] px-3 py-3 font-bold">
                  브라우저로 열기
                </button>
                <button onClick={copyAppUrl} className="rounded-xl border border-[#ead7ad] bg-white px-3 py-3 font-bold">
                  주소 복사
                </button>
              </div>
            </div>
